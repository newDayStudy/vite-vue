...
app.post("/getArticleList", (req, res) => {
  ...
  mysql(`SELECT count(*) as total FROM article_table`, (err, data) => {
    if (err) {
      res.json({
        data: 500,
        message: "总数量出错了",
      ...
      mysql(
        ...
        (err1, data1) => {
          ...
          mysql("SELECT id, name FROM USER_ROLE_TABLE", (e, d) => {
            if (e) {
              res.json({
                code: 500,
                message: "获取权限出错了",
...
app.post("/deleteArticle", (req, res) => {
  const { id } = req.body;
  mysql(`DELETE FROM article_table WHERE id = ${id}`, (err, data) => {
    if (err) {
      console.log(err);
      res.json({
        code: 500,
